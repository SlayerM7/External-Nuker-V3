        ...
        if (typeof res !== "undefined")
            if (typeof res.body !== "undefined") {
                if (res.body.message === "Missing Permissions") {
                    console.log(createString_1.createString(`Failed to ban ${mainColor(m)}`, "semi", "fail"));
                }
                    ...
                    console.log(createString_1.createString(`Banned ${mainColor(m)}`, "semi"));
                }
            }
    }));
}
function default_1(client, members, guildId) {
    let mainColor = getTheme_1.getTheme();
    members.map((m) => __awaiter(this, void 0, void 0, function* () {
        banMember(m, guildId, mainColor);
    }));
}
